...
# Ensure we can import from package root (src) regardless of where the script is placed
SRC_DIR = pathlib.Path(__file__).resolve().parent
if SRC_DIR.name == 'train':
...
def objective(trial):
    """
    The Optuna objective function for tuning Reward shaping.
    """
    # --- 1. Suggest Reward Parameters ---
    reward_kwargs = {
        'progress_factor': trial.suggest_float('progress_factor', 1.0, 8.0),
        'dist_weight': trial.suggest_float('dist_weight', 0.1, 2.0),
        'time_penalty': trial.suggest_float('time_penalty', -0.05, -0.001),
        'safety_factor': trial.suggest_float('safety_factor', -2.0, -0.1),
        'closing_speed_factor': trial.suggest_float('closing_speed_factor', 1.0, 10.0),
        'backward_reward_factor': trial.suggest_float('backward_reward_factor', 0.0, 0.05),
    }
    ...
    params = {
        ...
        'tau': 0.0038,
        'grad_clip': 1.6,
        # Fixed LRs from previous LR tuning phase
        'lr_actor': 3.3e-4,
        'lr_critic': 2.1e-4,
        'lr_alpha': 2.0e-4,
    }
    ...
    try:
        ...
        # Injecting dynamic reward weights into the environment
        env = PGRCEnv(reward_kwargs=reward_kwargs)
...
if __name__ == "__main__":
    # --- Argument Parser ---
    parser = argparse.ArgumentParser(description="Reward tuning with Optuna for SAC-LSTM.")
    parser.add_argument('--study-name', type=str, default="sac-lstm-reward-tuning", help="Name for the Optuna study.")
    ...
    best_params_file = "best_reward_parameters.txt"
